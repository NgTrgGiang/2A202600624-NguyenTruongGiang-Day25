...
class ResponseCache:
    ...
    def _tokenize(text: str) -> Counter[str]:
        """Bag-of-words vector: lowercase word tokens + character 3-grams per word."""
        tokens: list[str] = []
...
class SharedRedisCache:
    ...
    def get(self, query: str) -> tuple[str | None, float]:
        ...
        # Exact match by deterministic key hash.
        exact_key = f"{self.prefix}{self._query_hash(query)}"
        exact = self._redis.hget(exact_key, "response")
        if exact is not None:
            return exact, 1.0

        # Similarity scan over all stored queries under this prefix.
